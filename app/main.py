...
def healthz():
    ...
    Endpoint này phải **nhẹ**: không gọi Redis, không query DB, không nhận
    dependency nào. Nó chỉ trả lời câu hỏi "có cần restart container này
    không?". Nếu nó phụ thuộc Redis, Redis chết một nhịp là cả cụm container
    bị restart theo.
    """
