...
class ShutdownGuard:
    ...
    def start_draining(self, signum=None, frame=None) -> None:
        ...
        Chữ ký ``(signum, frame)`` là bắt buộc vì Python gọi handler với đúng
        2 tham số này. Không làm gì nặng ở đây (không gọi mạng, không ghi
        file) — handler chạy xen giữa bytecode.
        """
        self.draining = True

        # Mỗi tín hiệu chỉ có MỘT handler, nên đăng ký handler của mình là ghi
        # đè handler của uvicorn — thứ thật sự chịu trách nhiệm dừng server.
        # Không gọi lại nó thì app bật cờ "đang tắt" rồi chạy tiếp mãi, tới khi
        # orchestrator hết kiên nhẫn và SIGKILL. Đúng cái graceful shutdown
        # định tránh.
    ...
    def arm(self) -> None:
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            # Nhớ handler cũ TRƯỚC khi ghi đè, nếu không nó mất luôn và
            # start_draining không còn ai để nhường lại.
