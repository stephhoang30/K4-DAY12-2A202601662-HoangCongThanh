...
def emit(event: str, severity: str = "INFO", **fields) -> str:
    ...
    ``severity`` luôn viết hoa vì Google Cloud Logging chỉ nhận dạng dạng đó
    để tô màu và lọc. Không dùng ``indent``: cloud gom log theo dòng, một
    event xuống dòng là một log bị vỡ thành nhiều mẩu vô nghĩa.
